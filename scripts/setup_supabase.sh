...
# Setup storage after migrations
echo -e "${BLUE}🗄️  Setting up storage bucket and policies...${NC}"
if [ -f "supabase/storage.sql" ]; then
    supabase db sql -f supabase/storage.sql
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Storage setup completed${NC}"
    else
        echo -e "${YELLOW}⚠️  Storage setup may have encountered issues. This could be normal if bucket already exists.${NC}"
    fi
else
    echo -e "${YELLOW}⚠️  Storage SQL file not found at supabase/storage.sql${NC}"
fi
